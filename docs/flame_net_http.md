...
设置一个在每次请求处理过程开始前被执行的回调；`$cb` 原型如下：

``` PHP
// $req - 请求对象
// $res - 响应对象
// $match - 请求是否匹配了自定义路径处理器
function callback($req, $res, $match) {}
// 可以用于处理诸如登录判定、权限控制等
```
...
设置一个在每次请求处理过程接手后（请求、响应对象还未被销毁前）被执行的回调；`$cb` 原型与 `before` 相同（可以用于处理诸如日志记录、格式输出等）；
