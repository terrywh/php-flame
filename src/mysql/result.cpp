...
namespace flame::mysql
{
    ...
    //         if (!row.typeof(php::TYPE::ARRAY))
    //             return nullptr;
    //         if (field.typeof(php::TYPE::STRING))
    ...
    //             }
    //             return reinterpret_cast<MYSQL_RES*>(row); }, [this, co, ref, fetch_all, data_all](std::shared_ptr<MYSQL> c, MYSQL_RES *r, int error) mutable {
    //             MYSQL* conn = c.get();
    ...
    php::value result::fetch_all(php::parameters &params)
    {
        ...
        // row = cl_->fetch(conn, rs_, f_, n_, ch);
        // data.set(data.size(), row);
    ...
    //             mysql_free_result(r);
    //             return nullptr; }, [cl](std::shared_ptr<MYSQL> c, MYSQL_RES *r, int error) {
    //             // cl.reset() });
    // }
} // namespace flame::mysql
