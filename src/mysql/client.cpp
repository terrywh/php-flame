...
namespace mysql {
	void client::declare(php::extension_entry& ext) {
		...
		class_client
			...
			.method<&client::order>("order", {
				{"order", php::TYPE::UNDEFINED},
			})
			.method<&client::limit>("limit", {
				{"limit", php::TYPE::UNDEFINED},
			})
	...
	php::value client::where(php::parameters& params) {
		php::buffer buf;
		build_where(c_, buf, params[0]);
		return std::move(buf);
	}
	php::value client::order(php::parameters& params) {
		php::buffer buf;
		build_order(c_, buf, params[0]);
		return std::move(buf);
	}
	php::value client::limit(php::parameters& params) {
		php::buffer buf;
		build_limit(c_, buf, params[0]);
		return std::move(buf);
	}
