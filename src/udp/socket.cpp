...
PARSE_OPTION:
	...
	php::value socket::recv(php::parameters& params) {
		if(!connected_) throw php::exception(zend_ce_exception, "failed to send: socket not connected");
        ...
        if (err == boost::asio::error::operation_aborted || err == boost::asio::error::eof) return nullptr;
                                 ...
                                 (boost::format("failed to recv from socket: (%1%) %2%") % err.value() % err.message()).str(), err.value());
	...
	php::value socket::send(php::parameters& params) {
		if(!connected_) throw php::exception(zend_ce_exception, "failed to send: connected socket required");
        coroutine_handler ch{coroutine::current};
