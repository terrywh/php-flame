...
namespace flame::udp {
	...
	php::value connect(php::parameters& params) {
        ...
        if(pair.first.empty() || pair.second.empty()) throw php::exception(zend_ce_type_error, "failed to connect udp socket: illegal address format");

		coroutine_handler ch{coroutine::current};
        boost::system::error_code err;
		boost::asio::ip::udp::resolver::results_type eps;
		resolver_->async_resolve(pair.first, pair.second, [&err, &obj, &ptr, &eps, &ch] (const boost::system::error_code& error, boost::asio::ip::udp::resolver::results_type results) { // DNS 地址解析
			if(error) err = error;
			else eps = results;
			ch.resume();
		});
        ...
        boost::asio::async_connect(ptr->socket_, eps, [&err, &obj, &ptr, &ch](const boost::system::error_code &error, const boost::asio::ip::udp::endpoint &edp) { // 连接
            if (error) err = error;
			else {
        ...
        ch.suspend();
        if(err) throw php::exception(zend_ce_exception,
                (boost::format("failed to connect udp socket: (%1%) %2%") % err.value() % err.message()).str(), err.value());
		ptr->connected_ = true;
        return std::move(obj);
	}
