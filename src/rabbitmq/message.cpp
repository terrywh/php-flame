...
namespace flame::rabbitmq
{
    ...
    void message::build_ex(const AMQP::Message &msg, std::uint64_t tag)
    {
        ...
        if (msg.hasExpiration())
            set("expiration", msg.expiration());
        if (msg.hasReplyTo())
            set("reply_to", msg.replyTo());
        if (msg.hasCorrelationID())
            set("correlation_id", msg.correlationID());
        if (msg.hasPriority())
            set("priority", msg.priority());
        if (msg.hasDeliveryMode())
            set("delivery_mode", msg.deliveryMode());
        if (msg.hasHeaders())
            set("header", table2array(msg.headers()));
        if (msg.hasContentEncoding())
            set("content_encoding", msg.contentEncoding());
        if (msg.hasContentType())
            set("content_type", msg.contentType());
        if (msg.hasClusterID())
            set("cluster_id", msg.clusterID());
        if (msg.hasAppID())
            set("app_id", msg.appID());
        if (msg.hasUserID())
            set("user_id", msg.userID());
        if (msg.hasTypeName())
            set("type_name", msg.typeName());
        if (msg.hasTimestamp())
    ...
    void message::build_ex(AMQP::Envelope &env)
    {
        ...
        v = get("expiration");
        if (!v.empty())
            env.setExpiration(v.to_string());
        v = get("reply_to");
        if (!v.empty())
            env.setReplyTo(v.to_string());
        v = get("correlation_id");
        if (!v.empty())
            env.setCorrelationID(v.to_string());
        v = get("priority");
        if (!v.empty())
            env.setPriority(v.to_integer());
        v = get("delivery_mode");
        if (!v.empty())
            env.setDeliveryMode(v.to_integer());
        v = get("header");
        if (!v.empty())
            env.setHeaders(array2table(v));
        v = get("content_encoding");
        if (!v.empty())
            env.setContentEncoding(v.to_string());
        v = get("content_type");
        if (!v.empty())
            env.setContentType(v.to_string());
        v = get("cluster_id");
        if (!v.empty())
            env.setClusterID(v.to_string());
        v = get("app_id");
        if (!v.empty())
            env.setAppID(v.to_string());
        v = get("user_id");
        if (!v.empty())
            env.setUserID(v.to_string());
        v = get("type_name");
        if (!v.empty())
            env.setTypeName(v.to_string());
        v = get("timestamp");
        if (!v.empty())
            env.setTimestamp(v.to_integer());
        v = get("message_id");
        if (!v.empty())
            env.setMessageID(v.to_string());
    }
